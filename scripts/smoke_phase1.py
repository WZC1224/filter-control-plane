"""Phase 1 联调冒烟（真实 data818）：列表 → 建任务 → 查询 → 代理下载。

只连真实下游；不验号、不充值、不改 data818 代码。默认不创建任务：
  先对现有已完成任务试下载；传 --create 才走完整建/查/下闭环。

用法:
    python -m scripts.smoke_phase1 --base https://api.818gzs.ai
    python -m scripts.smoke_phase1 --create --fmt csv
    python -m scripts.smoke_phase1 --no-download   # 只读 + 查询，不碰文件
"""
...
def main() -> int:
    parser = argparse.ArgumentParser(description='Phase1 data818 smoke')
    parser.add_argument('--base', default=None, help='覆盖 DATA818_BASE_URL')
    parser.add_argument('--create', action='store_true', help='真创建一个新任务（消耗余额）')
    parser.add_argument('--filter-type', default='wsValid', help='建任务类型')
    parser.add_argument('--country', default='AD', help='建任务国家（默认最小量区）')
    parser.add_argument('--count', type=int, default=0, help='生成行数（默认取该类型 min_count）')
    parser.add_argument('--fmt', default='csv', choices=['csv', 'txt', 'xlsx', 'invalid'])
    parser.add_argument('--no-download', action='store_true', help='不试下载')
    parser.add_argument('--yes', action='store_true', help='跳过扣费确认（仅 --create 生效）')
    ...
    if args.create:
        ...
        if n < min_count or n > int(meta.get('max_count') or n):
            print(f'ABORT: count 必须在 {min_count}-{meta.get("max_count")}')
            return 2
        region = next(
        ...
        if not region:
            ...
            return 2
        if not args.yes:
            print(f'即将建单：{args.filter_type} {args.country} x{n}（按量计费，不可撤销）')
            print('重跑加 --yes 确认。')
            return 3
